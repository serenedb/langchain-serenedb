...
def test_hybrid_search_with_score(hybrid_store):
    # Scored search returns vector distances (not fused hybrid scores), so relevance
    # stays well-defined; hybrid ranking is exercised via similarity_search.
    res = hybrid_store.similarity_search_with_score("brown", k=2)
    assert len(res) == 2
    assert all(isinstance(score, float) for _, score in res)


def test_hybrid_scoring_is_rejected(hybrid_store):
    # Explicitly requesting a fused score is refused: fusion scores are rankings, not
    # distances.
    with pytest.raises(NotImplementedError):
        hybrid_store.similarity_search_with_score("brown", k=2, fts_query="brown")
...
def test_init_creates_hybrid_index():
    ...
    engine.init_vectorstore_table(
        table, DIM, overwrite_existing=True, hybrid_index_config=HybridIndexConfig()
    )
    vs = SereneDBVectorStore.create_sync(
        engine,
        DetEmb(),
...
def _run_fusion_strategy(fusion: FusionStrategy):
    ...
    vs.apply_hybrid_search_index()
    try:
        # similarity_search exercises the fused ranking for this strategy; scored search
        # returns vector distances only, so it is not used to test fusion here.
...
def test_hybrid_search_in_non_public_schema():
    ...
    engine.init_vectorstore_table(
        ...
        schema_name=schema,
        metadata_columns=[Column("category", "TEXT")],
        hybrid_index_config=HybridIndexConfig(),
    )
    vs = SereneDBVectorStore.create_sync(
        engine,
        DetEmb(),
...
def test_relevance_score_fn_selection():
    ...
    try:
        ...
        # Clamped to [0, 1]: float-epsilon overshoot and distant matches stay in range.
        assert fn_for(DistanceStrategy.COSINE_DISTANCE)(-1e-6) == 1.0
        assert fn_for(DistanceStrategy.COSINE_DISTANCE)(2.0) == 0.0
    finally:
        engine.drop_table(table)
        engine.close()
...
def test_relevance_scores_on_hybrid_store(hybrid_store):
    # On a hybrid store, relevance scoring uses vector distance (the scored path does not
    # fuse), so an exact match scores ~1.0 and scores descend -- fusion never leaks into
    # the relevance API.
    res = hybrid_store.similarity_search_with_relevance_scores(
        "the quick brown fox", k=3
    )
    assert res
    top_doc, top_score = res[0]
    assert top_doc.page_content == "the quick brown fox"
