...
from .hybrid_search_config import FusionStrategy, HybridIndexConfig, HybridSearchConfig
from .indexes import (
...
class AsyncSereneDBVectorStore(VectorStore):
    ...
    def _select_relevance_score_fn(self) -> Callable[[float], float]:
        ...
        The result is clamped to ``[0, 1]``: an exact match can yield a distance a hair
        below 0 (floating point), pushing ``1 - distance`` just above 1.0 and tripping the
        base class's range check, and distant matches can fall below 0.
        """
        base: Callable[[float], float]
        if self.distance_strategy == DistanceStrategy.COSINE_DISTANCE:
            base = self._cosine_relevance_score_fn
        elif self.distance_strategy == DistanceStrategy.EUCLIDEAN:
            base = self._euclidean_relevance_score_fn
        elif self.distance_strategy == DistanceStrategy.INNER_PRODUCT:
            base = self._max_inner_product_relevance_score_fn
        elif self.distance_strategy == DistanceStrategy.MANHATTAN:
            base = self._l1_relevance_score_fn
        else:
        ...
        def clamped(distance: float) -> float:
            return max(0.0, min(1.0, base(distance)))
    ...
    async def _aquery_collection(
        ...
        **kwargs: Any,
        ...
        # The full-text query is per-call: the search query text, or an explicit
        # ``fts_query`` kwarg. There is no store-level default (see HybridSearchConfig).
        fts_query = kwargs.get("fts_query", "")
    ...
    async def asimilarity_search(
        ...
        **kwargs: Any,
        ...
        # Default the full-text query to the search text; an explicit fts_query kwarg wins.
    ...
    @staticmethod
    def _reject_hybrid_scoring(kwargs: dict[str, Any]) -> None:
        """Refuse a per-call hybrid request on a scored path.

        Hybrid fusion produces a ranking score (higher = better, unbounded), not a
        distance, so returning it as a ``(doc, distance)`` pair -- and feeding it to the
        distance-based relevance function -- would be wrong. Scored search is always
        vector-distance; hybrid ranking is available via :meth:`asimilarity_search`.
        """
        if kwargs.get("fts_query") or kwargs.get("hybrid_search_config"):
            raise NotImplementedError(
                "Scored search does not support hybrid fusion: fused scores are rankings, "
                "not distances. Use similarity_search()/as_retriever() for hybrid "
                "ranking, or drop fts_query/hybrid_search_config for distance scores."
            )
    ...
    async def asimilarity_search_with_score(
        ...
        **kwargs: Any,
        ...
        """Return (document, distance) pairs by vector similarity.

        The score is the vector distance for the store's ``DistanceStrategy`` (smaller =
        better). Hybrid fusion is never applied on this scored path -- an explicit
        ``fts_query``/``hybrid_search_config`` is rejected -- so relevance scoring stays
        well-defined. For hybrid ranking use :meth:`asimilarity_search`.
        """
        embedding = await self.embedding_service.aembed_query(text=query)
    ...
    async def asimilarity_search_with_score_by_vector(
        ...
        **kwargs: Any,
        ...
        """Return (document, distance) pairs for a query vector.

        Scores are vector distances (smaller = better), so relevance scoring and the
        ``score_threshold`` retriever behave correctly. Hybrid fusion is intentionally
        not applied here (fused scores are rankings, not distances): an explicit
        ``fts_query``/``hybrid_search_config`` raises, and a store-level hybrid config is
        bypassed in favour of a dense query. Use :meth:`asimilarity_search` for hybrid.
        """
        self._reject_hybrid_scoring(kwargs)
        rows = await self._aquery_collection(
            embedding, k=k, filter=filter, hybrid_search_config=None
        )
    ...
    async def _acreate_text_search_dictionary(self, config: HybridIndexConfig) -> None:
    ...
    async def _abuild_index(
        self, index: BaseIndex, index_config: Optional[HybridIndexConfig]
    ) -> None:
        """Create the collection's single inverted index on the embedding column.

        The index name is derived from the table (:attr:`_vector_index_name`) — one index
        per collection, fully controlled by the store. When ``index_config`` is given, a
        single combined inverted index is built over both the content column (analyzed
        with its BM25-capable dictionary) and the embedding column, with the id stored via
        ``INCLUDE`` so the lexical branch can return it; otherwise a plain vector index.
        """
        ...
        if index_config is not None:
            await self._acreate_text_search_dictionary(index_config)
            stmt = build_hybrid_index_ddl(
                ...
                dictionary_name=index_config.dictionary_name,
    ...
    async def aapply_vector_index(
        ...
        *,
        ...
        """Create the collection's single plain vector (IVF) index on the embedding column.

        For the combined full-text + vector index, use
        :meth:`aapply_hybrid_search_index`.
        """
        await self._abuild_index(index, index_config=None)

    async def aapply_hybrid_search_index(
        self,
        index: Optional[BaseIndex] = None,
        index_config: Optional[HybridIndexConfig] = None,
        concurrently: bool = False,
    ) -> None:
        """Create the combined full-text + vector inverted index for hybrid search.
        ...
        ``index_config`` (a :class:`~langchain_serenedb.hybrid_search_config.HybridIndexConfig`)
        selects the text search dictionary; it defaults to ``HybridIndexConfig()``.
        """
        if index is None:
            from .indexes import IVFIndex

            index = IVFIndex(distance_strategy=self.distance_strategy)
        await self._abuild_index(
            index, index_config=index_config or HybridIndexConfig()
        )

    async def areindex(self) -> None:
        """Rebuild the store's vector/inverted index in place. Not supported yet.

        SereneDB's ``REINDEX`` currently applies to views only, not to the base table the
        store lives in, so there is no way to rebuild this index in place. This raises
        :class:`NotImplementedError` for now; it will perform a real reindex once SereneDB
        supports ``REINDEX`` on tables. To rebuild today, drop and recreate the index
        (:meth:`adrop_vector_index` + :meth:`aapply_vector_index`).
        """
        raise NotImplementedError(
            "SereneDB REINDEX currently supports views only, not the store's base table, "
            "so reindexing in place is not available yet. Drop and recreate the index "
            "instead (drop_vector_index + apply_vector_index)."
        )
