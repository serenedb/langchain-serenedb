...
from .hybrid_search_config import HybridIndexConfig, HybridSearchConfig
from .indexes import (
...
class SereneDBVectorStore(VectorStore):
    ...
    def apply_hybrid_search_index(
        ...
        *,
        ...
        return self._engine._run_as_sync(
            self.__vs.aapply_hybrid_search_index(
                index, index_config=index_config, concurrently=concurrently
            )
        )
    ...
    def reindex(self) -> None:
        ...
        Sync wrapper for :meth:`AsyncSereneDBVectorStore.areindex`; raises
        :class:`NotImplementedError` until SereneDB supports ``REINDEX`` on tables (it
        currently applies to views only). To rebuild today, use
        :meth:`drop_vector_index` + :meth:`apply_vector_index`.
        """
